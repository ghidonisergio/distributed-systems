...
<%@ page import="it.distributedsystems.model.ejb.EntityController" %>
...
<html>
	...
	<%
		...
		out.println("<!-- inserted producer '" + producer.getName() + "', with id = '" + id + "' -->");
				...
				EntityController ec = EntityController.getEntityController();
				
				ec.insertProductWithProducer("myproducer", "myproduct", 21);
		...
		product.setProductNumber(Integer.parseInt(request.getParameter("number")));
			...
			if (producerId != null) {
				Integer idp = Integer.parseInt(producerId);
				Producer producer = producerDAO.findProducerById(idp);
				//Producer producer = new Producer();
				//producer.setName( "myproducer" );
			
				/*
				è tutto inutile: dà cmq detahced entity...  e per ovvi motivi visto
				che il em che recuperi dal ServletContext NON è lo stesso em usato dai bean
				e iniettato con @PersistenceContext
				EntityManager em = (EntityManager) application.getAttribute("em");
				UserTransaction utx = (UserTransaction) application.getAttribute("utx");
				
				utx.begin();
				em.joinTransaction();// --> con o senza lui non cambia (il join trans. sembra automa.)
				em.merge(producer);*/
